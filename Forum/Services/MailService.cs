...
namespace Forum.Services
{
    public class MailService
    {
        ...
        //將使用者資料填入驗證信範本中
        public string GetRegisterMailBody(string TempString, string UserName,
            string ValidateUrl)
        {
            //將使用者資料填入
        ...
        public void SendRegisterMail(string MailBody, string ToEmail)
        {
            ...
            SmtpServer.Port = 587; //設定使用的Port，這裡設定Gmail所使用的
            //建立使用者憑據，這裡要設定您的Gmail帳戶
            SmtpServer.Credentials = new System.Net.NetworkCredential(gmail_account, gmail_password);
            SmtpServer.EnableSsl = true; //開啟SSL 
            MailMessage mail = new MailMessage(); //宣告信件內容物件
            mail.From = new MailAddress(gmail_mail); //設定來源信箱
            mail.To.Add(ToEmail); //設定收信者信箱 
            mail.Subject = "會員註冊確認信"; //設定信件主旨
